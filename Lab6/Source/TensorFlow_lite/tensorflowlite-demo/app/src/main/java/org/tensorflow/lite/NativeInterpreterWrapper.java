...
final class NativeInterpreterWrapper implements AutoCloseable {
  ...
  private static native long createErrorReporter(int size);//1、设置错误报告器的大小

  private static native long createModel(String modelPathOrBuffer, long errorHandle);//2、由文件创建网络模型

  private static native long createModelWithBuffer(MappedByteBuffer modelBuffer, long errorHandle);//2.2、由内存模型数据创建

  private static native long createInterpreter(long modelHandle);//3、创建tf实例
  ...
  private static native long[] run(
      ...
      int[] dtypes,
      int[] numsOfBytes,
      Object[] values);//4、运行网络预测

  private static native void delete(long errorHandle, long modelHandle, long interpreterHandle);//5、删除实例、模型等
