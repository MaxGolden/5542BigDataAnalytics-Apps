...
object ImageClassification_DT {
  def main(args: Array[String]) {
    val IMAGE_CATEGORIES = Array("running", "dunk", "standing")
//    System.setProperty("hadoop.home.dir", "C:\\Users\\Manikanta\\Documents\\UMKC Subjects\\PB\\hadoopforspark")
//    System.setProperty("hadoop.home.dir", "C:\\HADOOP_HOME\\bin")
    // Turn off Info Logger for Consolexxx
    Logger.getLogger("org").setLevel(Level.OFF)
    Logger.getLogger("akka").setLevel(Level.OFF)
    val sparkConf = new SparkConf().setAppName("ImageClassification").setMaster("local[*]")
    val sc = new SparkContext(sparkConf)
    val train = sc.textFile("ImageClassification\\data\\train1")
    val test = sc.textFile("ImageClassification\\data\\test1")
    ...
    val mat = metrics.confusionMatrix

    println("Accuracy:" + metrics.accuracy)
    println("Confusion Matrix:")
    println(metrics.confusionMatrix)

    val server = HttpServer.create(new InetSocketAddress("192.168.1.183", 8080), 0)
...
//      val response = "Text being sent from Apache spark server"
      val response = "Accuracy:" + acc+"\n\nConfusion Matrix:\n" + mat
